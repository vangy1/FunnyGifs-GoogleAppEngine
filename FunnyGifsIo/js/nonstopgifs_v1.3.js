...
function getVideos(min,max){
	$.ajax({
	url: "http://funnygifs.media/getposts_alternate?minimum="+min+"&maximum="+max, 
...
function resizeHeights(){
	...
	if (availableGifCardHeight > 700){availableGifCardHeight = 700;}
	$('#gif-card-container').css('height',availableGifCardHeight + 'px');
	var availableVideoHeight = availableGifCardHeight - $('.video-title').outerHeight() - $('.share-controls-inner').outerHeight();
...
function insertNextButton(){
	$('.card').append(
		'<a href="/nonstopgifs.php?utm_source=next_button&utm_medium=next_button"><div id="next"><span class="helper"></span>'+
		'<div id="next-helper">'+
